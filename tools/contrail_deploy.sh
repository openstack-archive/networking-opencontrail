...
EOF
	...
	ansible-playbook -vv -e skip_openstack=true -i inventory/ playbooks/provision_instances.yml

	[ $? -ne 0 ] && { echo "-> Provision instances fail, aborting - will not install contrail"; exit 2; }
	echo '-> Run ansible (install_contrail)...'
	ansible-playbook -vv -e skip_openstack=true -e '{"CREATE_CONTAINERS":true}' -e orchestrator=none -i inventory/ playbooks/install_contrail.yml
	echo '-> DONE!'
}
