...
class SubnetDNSCompatibilityIntegratorTestCase(testlib_api.SqlTestCase):
    ...
    def test_network_abort_delete_callback(self):
        ...
        self.dns_integrator.synchronize_dns_port_for_subnet.assert_has_calls(
            expected_integrator_calls)

    def _mock_core_plugin_get_port(self, ports):
        self.core_plugin.get_ports = mock.Mock(return_value=ports)

    def _mock_core_plugin_get_subnets(self, subnets):
        results = [{'id': subnet['id']} for subnet in subnets]
        self.core_plugin.get_subnets = mock.Mock(return_value=results)

    def _mock_tf_driver_get_subnet(self, subnet_data):
        if isinstance(subnet_data, list):
            self.tf_driver.get_subnet = mock.Mock(side_effect=subnet_data)
        else:
