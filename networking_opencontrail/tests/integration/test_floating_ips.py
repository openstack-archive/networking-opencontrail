...
class TestFloatingIPs(IntegrationTestCase):
    ...
    def test_create_floatingip(self):
        ...
        expected = {
            'floating_ip_address': q_floating_ip.get(['floatingip']['floating_ip_address']),
            'project_id': uuid.UUID(q_floating_ip.get(['floatingip']['project_id'])),
            'uuid': uuid.UUID(q_floating_ip.get(['floatingip']['id']))
        }

        tf_floating_ip = self.tf_get_resource('floating-ip', q_floating_ip['floatingip']['id'])
        self.assertIsNotNone(tf_floating_ip)

        contrail_dict = {
            'floating_ip_address': tf_floating_ip.get(['floating_ip_address']),
            'project_id': uuid.UUID(tf_floating_ip.get(['project_refs'][0]['uuid'])),
    ...
    def test_delete_floatingip(self):
        ...
        floating_ip_req = self.tf_request_resource('floating-ip', q_floating_ip['floatingip']['id'])
        self.assertEqual(floating_ip_req.status_code, 404)
