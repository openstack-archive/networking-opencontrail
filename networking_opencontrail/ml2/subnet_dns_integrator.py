...
class SubnetDNSCompatibilityIntegrator(object):
    ...
    def after_create_subnet(self, resource, event, trigger, **kwargs):
        context = kwargs.get('context')
        subnet = {'subnet': kwargs.get('subnet')}

        try:
            ret_subnet = self.tf_driver.create_subnet(context, subnet)
            self.add_dns_port_for_subnet(context, ret_subnet)
        except Exception:
            with excutils.save_and_reraise_exception():
    ...
    def subscribe(self):
        ...
        registry.subscribe(
            ...
            events.ABORT_DELETE)
        registry.subscribe(
            self.after_create_subnet, resources.SUBNET, events.AFTER_CREATE)
