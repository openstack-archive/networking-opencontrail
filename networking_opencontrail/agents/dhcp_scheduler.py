...
class TFIgnoreDHCPScheduler(base_scheduler.BaseChanceScheduler,
    ...
    def schedule(self, plugin, context, resource):
        ...
        if self.resource_filter.check_network_exist_in_tf(network_id):
            LOG.info("Network %s is managed by Tungsten Fabric:"
                     " do not schedule any DHCP agent." % network_id)
            return []
...
class TFDHCPFilter(dhcp_agent_scheduler.DhcpFilter):
    ...
    def bind(self, context, agents, network_id):
        if self.check_network_exist_in_tf(network_id):
            LOG.info("Network %s is managed by Tungsten Fabric:"
                     " do not schedule any DHCP agent." % network_id)
            return
    ...
    def check_network_exist_in_tf(self, network_id):
